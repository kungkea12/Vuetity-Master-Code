<template>
    <v-card color="white" class="rounded-lg" flat>
        <v-sheet tile height="54" class="d-flex">
            ...
            <v-spacer></v-spacer>
            <v-btn icon class="ma-2" @click="$refs.calendar">
                <v-icon>mdi-chevron-right</v-icon>
            </v-btn>
        </v-sheet>
        <v-sheet height="600">
            <v-calendar ref="calendar" v-model="value" :weekdays="weekday" :type="type" :events="events" :event-overlap-mode="mode"
            :event-overlap-threshold="30" :event-color="getEventColor" @change="getEvents"></v-calendar>
        </v-sheet>
    </v-card>
</template>
